...
public class PrinterService extends CordovaPlugin {
    ...
    public boolean execute(String action, CordovaArgs args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("connectNetworkPrinters")) {
            String ipAddress = args.getString(0);
            connectNetworkPrinters(ipAddress, callbackContext);
            return true;
        }
    ...
    private void connectNetworkPrinters(String ipAddress, final CallbackContext callbackContext) {
        JSONArray printers = new JSONArray();
        JSONObject jsonObj = new JSONObject();
        if(connFlag == 0){
            connFlag = 1;
